...
#   STORM_MASTER    host:path where storm code should be rsync'd from
#   STORM_PID_DIR   The pid files are stored. /tmp by default.
#   STORM_IDENT_STRING   A string representing this instance of storm. $USER by default
#   STORM_NICENESS The scheduling priority for daemons. Defaults to 0.
##
...
if [ "$STORM_IDENT_STRING" = "" ]; then
  export STORM_IDENT_STRING="$USER"
fi
...
case $startStop in
  ...
  (start)
    ...
    if [ "$STORM_MASTER" != "" ]; then
      echo rsync from $STORM_MASTER
      rsync -a -e ssh --delete --exclude=.svn --exclude='logs/*' --exclude='contrib/hod/logs/*' $STORM_MASTER/ "$STORM_HOME"
    fi
