...
for f in $STORM_HOME/storm-core-java-*.jar; do
  CLASSPATH=${CLASSPATH}:$f;
done

for f in $STORM_HOME/external/storm-drpc/storm-drpc-*.jar; do
  CLASSPATH=${CLASSPATH}:$f;
done

for f in $STORM_HOME/external/storm-ui/storm-ui-*.jar; do
  CLASSPATH=${CLASSPATH}:$f;
done

for f in $STORM_HOME/external/storm-scheduler/storm-scheduler-*.jar; do
  CLASSPATH=${CLASSPATH}:$f;
done
...
if [ -d "${STORM_HOME}/build/native" -o -d "${STORM_HOME}/lib/native" ]; then
  JAVA_PLATFORM=`CLASSPATH=${CLASSPATH} ${JAVA} -Xmx32m ${STORM_JAVA_PLATFORM_OPTS} org.apache.storm.utils.PlatformName | sed -e "s/ /_/g"`
